...
def plot_heatmap_def():
    ...
    genes_in_data_set = set(adata_sub2.raw.var.gene_symbols)   # extrahiert die Liste von vorhandenen Genen aus dem dataset
...
def plot_umap():
    ...
    if ((genes_to_plot not in ['predictions', 'orig.ident', 'condition', 'study']) and (genes_to_plot not in adata_to_sub.raw.var.gene_symbols)):
    ...
    if (subset != 'Do not subset') and ('adata_sub' in globals() and (subset not in adata_sub.obs.mnc_lineage.values)):
        ...
        sc.tl.leiden(adata_sub, resolution = 1.0)
        print(colored('...done!', 'cyan'))
        if ((genes_to_plot not in ['predictions', 'orig.ident', 'condition', 'study']) and (genes_to_plot not in adata_sub.raw.var.gene_symbols)):
            sc.pl.umap(adata_sub, color=[genes_to_plot], size=40, legend_loc="on data")
